...
import { ILocationArguments, IMapFeatures } from '../../index';
...
export class WebMapLocationComponent {
	...
	onBindPlaces = (
		...
		args.apiOptions = new ApiServiceParametersOptions();
		args.apiOptions.url = Config.ENVIRONMENT()
			.MAP_BOX_API.concat('{query}')
			.concat('.json?access_token=')
			.concat(Config.ENVIRONMENT().MAP_BOX_API_KEY); //Config.API_END_POINTS.INDUSTRIES.concat('{query}');
		args.apiOptions.parameters = '';
	...
	onLocate(args: ILocationArguments) {
		return this.location.onSearch(args) as Observable<IKeyValueDictionary>;
	}
}
