...
export class IssuesProcessor {
    static mapIssues(data: Array<any>): Array<Issue> {
        return data
            .reduce<Array<any>>(
                (agg: Array<any>, curr: Array<any>) => [...agg, ...curr],
                []
            )
    ...
    static mapIssue(issue: any): Issue {
        return {
            ...
            assignees: issue.assignees
                ? issue.assignees.map(IssuesProcessor.mapUser)
                : [],
            state: issue.state,
    ...
    process(data, months) {
        const mappedIssues = this.filterByMonth(
            IssuesProcessor.mapIssues(data),
            months
        );
    ...
    closeRate(data) {
        ...
        return {
            lowest: rate.reduce(
                (agg, curr) => (agg.close_rate < curr.close_rate ? agg : curr)
            ),
            highest: rate.reduce(
                (agg, curr) => (agg.close_rate > curr.close_rate ? agg : curr)
            ),
            average:
                data.closed.length / (data.open.length + data.closed.length)
        };
    }
    ...
    groupLabels(data) {
        const labels = this.aggregate(
            this.flatten(data.map(item => item.labels)),
            'name'
        );
    ...
    filterByMonth(data: Array<Issue>, months): Array<Issue> {
        return data.filter(value => {
            return (
                ...
                this.getMonthsRange(months).getTime()
            );
        });
    }
